...
function CrosswordPlayer({puzzle,onBack}){
  ...
  // entries that pass through a given cell
  const entriesThrough=(r,c)=>{
    const out=[];
    [...across,...down].forEach(e=>{
      for(let i=0;i<e.a.length;i++){
        const er=e.dir==='across'?e.r:e.r+i, ec=e.dir==='across'?e.c+i:e.c;
        if(er===r&&ec===c){ out.push(e); break; }
      }
    });
    return out;
  };
  const lockCompleted=(ng,r,c)=>{
    const newLocks={};
    entriesThrough(r,c).forEach(e=>{
      let correct=true;
      for(let i=0;i<e.a.length;i++){
        const er=e.dir==='across'?e.r:e.r+i, ec=e.dir==='across'?e.c+i:e.c;
        if((ng[er][ec]||'')!==e.a[i]){ correct=false; break; }
      }
      if(correct){ for(let i=0;i<e.a.length;i++){ const er=e.dir==='across'?e.r:e.r+i, ec=e.dir==='across'?e.c+i:e.c; newLocks[er+'_'+ec]=true; } }
    });
  ...
  const type=(ch)=>{
    ...
    setGrid(ng);
    lockCompleted(ng, sel.r, sel.c);
    // advance to next non-locked cell
    let nr=dir==='across'?sel.r:sel.r+1, nc=dir==='across'?sel.c+1:sel.c;
    while(isCell(nr,nc)&&isLocked(nr,nc)){ nr=dir==='across'?nr:nr+1; nc=dir==='across'?nc+1:nc; }
      ...
      <div style={{display:'flex',justifyContent:'center',marginBottom:16,overflowX:'auto'}}>
        <div style={{display:'grid',gridTemplateColumns:'repeat('+Cn+', '+cellPx+'px)',gap:1,background:'#1a1a1a',padding:1,borderRadius:2}}>
          {cell.map((row,r)=>row.map((x,c)=>{
            ...
            const lock=isLocked(r,c);
            const val=grid[r][c];
            const wrong=checked&&val&&val!==x.sol;
            const bg=isSel?C.teal:lock?'#fff':active?'#EAEDEF':'#fff';
            const letterCol=lock?C.green:wrong?C.red:isSel?C.dark:'#111';
            return(
              <div key={r+'_'+c} onClick={()=>pick(r,c)} style={{width:cellPx,height:cellPx,background:bg,position:'relative',display:'flex',alignItems:'center',justifyContent:'center',cursor:lock?'default':'pointer'}}>
                ...
                <span style={{fontSize:cellPx*0.55,fontWeight:700,color:letterCol}}>{val}</span>
              </div>
            );
          }))}
        </div>
...
const DRAFT_POS={
  // GK
  'Alisson':'GK','David Raya':'GK','Pickford':'GK','Sanchez':'GK','Vicario':'GK','Pope':'GK','Martinez':'GK','Sels':'GK','Henderson':'GK','Courtois':'GK','Oblak':'GK','Maignan':'GK','Neuer':'GK','Sommer':'GK','Di Gregorio':'GK','Meret':'GK','Kobel':'GK','Verbruggen':'GK','Areola':'GK','Petrovic':'GK','Szczesny':'GK','Rulli':'GK','Provedel':'GK','Carnesecchi':'GK',
  // DEF
  'Van Dijk':'CB','Saliba':'CB','Gabriel':'CB','Dias':'CB','Gvardiol':'CB/LB','Trippier':'RB','Robertson':'LB','White':'RB/CB','Cucurella':'LB','James':'RB','Romero':'CB','Van de Ven':'CB','Konate':'CB','Stones':'CB/RB','Burn':'LB/CB','Mings':'CB','Tarkowski':'CB','Colwill':'CB','Botman':'CB','Rudiger':'CB','Carvajal':'RB','Cubarsi':'CB','Kounde':'RB/CB','Bastoni':'CB','Bremer':'CB','Hakimi':'RB','Davies':'LB','Kim Min-jae':'CB','Tah':'CB','Schlotterbeck':'CB','Araujo':'CB/RB','Balde':'LB','Dimarco':'LB','Di Lorenzo':'RB','Bensebaini':'LB/CB','Marquinhos':'CB','Mendes':'LB','Aina':'RB/LB','Munoz':'RB','Lacroix':'CB','Kerkez':'LB','Frimpong':'RB','Murillo':'CB','Livramento':'RB','Hall':'LB','Pau Torres':'CB','Digne':'LB','Estupinan':'LB','Tomori':'CB','Calafiori':'CB/LB','Timber':'RB/CB','Acerbi':'CB','Gatti':'CB','Buongiorno':'CB','Ryerson':'RB','Anton':'CB','Upamecano':'CB',
  // MID
  'Rodri':'CDM','Rice':'CM','Mac Allister':'CM','Szoboszlai':'CM','Fernandes':'CAM','Palmer':'CAM','Maddison':'CAM','Caicedo':'CDM','Gravenberch':'CM','Wharton':'CM','Bruno Guimaraes':'CM','Tonali':'CM','Amadou Onana':'CM','Mainoo':'CM','Rogers':'CAM','Bellingham':'CM/CAM','Valverde':'CM','Tchouameni':'CDM','Pedri':'CM','Gavi':'CM','De Jong':'CM','Barella':'CM','Calhanoglu':'CDM','Vitinha':'CM','Musiala':'CAM','Kimmich':'CDM/RB','McTominay':'CM','Lobotka':'CDM','Pellegrini':'CAM','Koopmeiners':'CM/CAM','Olise':'RW','Fabian Ruiz':'CM','Zubimendi':'CDM','Ugarte':'CDM','Frattesi':'CM','Brandt':'CAM','Sangare':'CDM','Anderson':'CM','Smith Rowe':'CAM','Garner':'CM','Gibbs-White':'CAM','Kluivert':'CAM/RW','Sarr':'CAM','Merino':'CM','Modric':'CM','Loftus-Cheek':'CM','Mkhitaryan':'CM','Locatelli':'CDM','Khephren Thuram':'CM','Anguissa':'CM','Sabitzer':'CM','Goretzka':'CM','Casado':'CDM',
  // FWD
  'Haaland':'ST','Salah':'RW','Saka':'RW','Foden':'CAM/LW','Watkins':'ST','Gakpo':'LW','Cunha':'ST/CAM','Mbeumo':'RW','Wood':'ST','Mateta':'ST','Bowen':'RW','Vinicius':'LW','Mbappe':'ST/LW','Rodrygo':'RW/LW','Lewandowski':'ST','Raphinha':'LW','Yamal':'RW','Lautaro':'ST','Thuram':'ST','Vlahovic':'ST','Leao':'LW','Kane':'ST','Dembele':'RW/ST','Kvaratskhelia':'LW','Griezmann':'ST/CAM','Williams':'LW','Adeyemi':'LW','Guirassy':'ST','Barcola':'LW','Marmoush':'ST','Schade':'RW','Welbeck':'ST','Mitoma':'LW','Joao Pedro':'ST','Pedro Neto':'RW','Solanke':'ST','Kudus':'RW','Elanga':'RW','Nketiah':'ST','Julian Alvarez':'ST','Ferran Torres':'LW/ST','Nico Williams':'LW','Oyarzabal':'ST/LW','Gimenez':'ST','Pulisic':'LW/RW','Chiesa':'RW','Gyokeres':'ST','Beto':'ST','Ekitike':'ST',
};
...
function PlayerCard({player, w}){
  ...
  const baseW=w||150;
  const W=leg?Math.round(baseW*1.12):baseW;
  const st=player.stats||deriveStats(player, player.group||'MID');
  const H=W*1.46;
  const accent=leg?'#ffe9b8':'#ffffff';
  const sub=leg?'#f0d2ff':'#bfe2ff';
  const body=leg
    ? 'linear-gradient(165deg,#3d1268 0%,#5a1e9e 45%,#2a0d4d 100%)'
    : 'radial-gradient(120% 80% at 50% 18%,#2a86e8 0%,#13478f 45%,#071f48 100%)';
  // frames: gold for legend, silver for current
  const frame=leg?'linear-gradient(#ffe9a8,#caa23f)':'linear-gradient(160deg,#f5f9ff,#aebfcf 45%,#6d7f92)';
  const topbar=leg?'linear-gradient(90deg,#caa23f,#ffe9a8,#caa23f)':'linear-gradient(90deg,#cfe6ff,#8fbfe8,#cfe6ff)';
  const trim=leg?'rgba(255,220,150,.35)':'rgba(210,228,245,.4)';
  ...
  return(
    <div style={{width:W,margin:'0 auto',padding:3,background:frame,clipPath:clip,boxShadow:glowShadow}}>
      <div style={{height:H,display:'flex',flexDirection:'column',overflow:'hidden',background:body,clipPath:clip,position:'relative'}}>
        {/* electric light shards (current only) */}
        {!leg&&<div style={{position:'absolute',inset:0,overflow:'hidden',pointerEvents:'none'}}>
          <div style={{position:'absolute',top:-20,left:'8%',width:W*0.22,height:'150%',background:'linear-gradient(rgba(120,200,255,.28),rgba(120,200,255,0))',transform:'rotate(20deg)'}}/>
          <div style={{position:'absolute',top:-20,left:'26%',width:W*0.09,height:'150%',background:'linear-gradient(rgba(150,220,255,.42),rgba(150,220,255,0))',transform:'rotate(20deg)'}}/>
          <div style={{position:'absolute',top:-20,right:'14%',width:W*0.28,height:'150%',background:'linear-gradient(rgba(120,200,255,.2),rgba(120,200,255,0))',transform:'rotate(20deg)'}}/>
          <div style={{position:'absolute',top:-20,right:'4%',width:W*0.06,height:'150%',background:'linear-gradient(rgba(180,235,255,.5),rgba(180,235,255,0))',transform:'rotate(20deg)'}}/>
        </div>}
        {/* legend gloss sweep */}
        {leg&&<div style={{position:'absolute',inset:0,background:'linear-gradient(115deg,rgba(255,255,255,0) 33%,rgba(255,255,255,.3) 48%,rgba(255,255,255,0) 60%)',pointerEvents:'none'}}/>}
        {/* top sheen + bar */}
        <div style={{position:'absolute',top:0,left:0,right:0,height:'48%',background:'linear-gradient(120deg,rgba(255,255,255,.16),rgba(255,255,255,0) 65%)',pointerEvents:'none'}}/>
        <div style={{position:'absolute',top:0,left:0,right:0,height:Math.max(3,W*0.03),background:topbar}}/>
        <div style={{position:'absolute',top:3,left:3,right:3,bottom:3,border:'1px solid '+trim,clipPath:clip,pointerEvents:'none'}}/>
        ...
        <div style={{position:'relative',height:'100%',display:'flex',flexDirection:'column'}}>
          ...
          <div style={{display:'flex',alignItems:'flex-start',padding:(W*0.07)+'px '+(W*0.08)+'px 0'}}>
            <div style={{textAlign:'center',lineHeight:1.05,minWidth:W*0.22}}>
              <div style={{fontFamily:'Bebas Neue,sans-serif',fontSize:W*0.2,color:accent,textShadow:leg?'0 0 8px rgba(255,210,120,.6)':'0 1px 4px rgba(0,40,90,.6)'}}>{player.r}</div>
              <div style={{fontSize:W*0.066,fontWeight:700,color:sub}}>{DRAFT_POS[player.n]||player.pos||''}</div>
...
function Quiz({openPlayer, openClub, onExit, initialSection}){
  const [section,setSection]=useState(initialSection||'quizzes'); // quizzes | crosswords
  ...
  return(
    <div style={{position:'relative',width:W,height:H,flexShrink:0}}>
      <svg width={W} height={H} viewBox={"0 0 "+W+" "+H} style={{position:'absolute',top:0,left:0}}>
        ...
        <path d={"M "+(W*0.34)+" "+(H*0.2+6)+" A "+(W*0.2)+" "+(W*0.2)+" 0 0 0 "+(W*0.66)+" "+(H*0.2+6)} fill="none" stroke={lc} strokeWidth="1.2"/>
        ...
        <path d={"M "+(W*0.34)+" "+(H-6-H*0.2)+" A "+(W*0.2)+" "+(W*0.2)+" 0 0 1 "+(W*0.66)+" "+(H-6-H*0.2)} fill="none" stroke={lc} strokeWidth="1.2"/>
...
function MatchModal({match, onClose, openPlayer, openClub, initialTab, initialPitchTeam, onStateChange}){
  ...
  const summaryItems = (finished && events && events.length>0) ? (()=>{
    ...
    const sideOf=(e)=>{
      if(e.team?.id!=null&&match.homeTeam?.id!=null) return e.team.id===match.homeTeam.id?'h':'a';
      const en3=norm(e.team?.name||''); return en3.length>0&&(en3.includes(hn3.slice(0,5))||hn3.includes(en3.slice(0,5)))?'h':'a';
    };
          ...
          {tab==='events'&&(
            <div>
              ...
              {!afLoading&&events&&events.length>0&&[...events].sort((a,b)=>a.time?.elapsed-b.time?.elapsed).map((e,i)=>{
                ...
                const isHome=(e.team?.id!=null&&match.homeTeam?.id!=null)
                  ? e.team.id===match.homeTeam.id
                  : (en3.length>0&&(en3.includes(hn3.slice(0,5))||hn3.includes(en3.slice(0,5))));
